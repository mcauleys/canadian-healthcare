...
Rework dataframe to summarize by individual companies
```{r}
df.company <- df %>% group_by(tick, cat, ret) %>% summarise(count = n(), total = sum(val))
```
...
Rework dataframe to summarize by individual companies without Bausch
```{r}
df.company <- df %>% group_by(tick, cat) %>% summarise(count = n(), total = sum(val))

mean(df.company$count)
median(df.company$count)

mean(df.company$total)
median(df.company$total)

```

Rework dataframe to summarize by individual investors
```{r}
df.investor <- df %>% group_by(investors, loc) %>% summarise(count = n(), total = sum(val), w.mean_return = weighted.mean(ret, val))
```

Overall summary
```{r}
summary(df.investor)
length(unique(df.investor$investors))
unique(df.investor$loc)

sum(df.investor$total)
...
sum(df.investor$total[!(df.investor$loc == "United States" | df.investor$loc == "Canada")])
mean(df.investor$total[!(df.investor$loc == "United States" | df.investor$loc == "Canada")])
median(df.investor$total[!(df.investor$loc == "United States" | df.investor$loc == "Canada")])
...
tmp <- arrange(df.investor[df.investor$loc == "Canada",], desc(w.mean_return))
tmp <- as.character(tmp$investors[1:10])

png(file = "canadian-investors3.png", width = 15, height = 6, units = 'in', res = 300)
ggplot(df[df$investors %in% tmp,]) +
